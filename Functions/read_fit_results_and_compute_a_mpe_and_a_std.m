function  [results] = read_fit_results_and_compute_a_mpe_and_a_std(path_of_script,voxel_folder,simulated_SNRs,number_of_noise_realizations,slice,results)
 ...
 accuracy_threshold = 0.05; % kritischer Wert ab dem die Mittelwerte die GT als gut geschätzt ansehen 0.05 = 5% Differenz zur GT
  ...
  for inx_voxel = 1:size(voxel_folder,2)
        ...
        for index_SNR = simulated_SNRs
              ...
              for AxTM_index = 1:numel(AxTM)
                  ...
                  AxTM_map_name_1 = ['SNR_' num2str(index_SNR)  '_simulation_standard_DKI_001_desc-NLLS-AxDKI-' AxTM{AxTM_index} '_dwi.nii'];
                  AxTM_map_name_2 = ['SNR_' num2str(index_SNR)  '_simulation_standard_DKI_001_desc-DKI-NLLS-' AxTM{AxTM_index} '_dwi.nii'];
                  AxTM_map_name_3 = ['SNR_' num2str(index_SNR)  '_simulation_standard_DKI_001_desc-DKI-NLLS-RBC-' AxTM{AxTM_index} '_dwi.nii'];
                  AxTM_map_name_4 = ['SNR_' num2str(index_SNR)  '_simulation_standard_DKI_001_desc-NLLS-AxDKI-RBC-' AxTM{AxTM_index} '_dwi.nii'];
                 ...
                 standardDKI_AxTMs_tmp = acid_read_vols(Vol{2,AxTM_index},Vol{1,1},-4); 
                 standardDKI_RBC_AxTMs_tmp = acid_read_vols(Vol{3,AxTM_index},Vol{1,1},-4); 
                 AxDKI_AxTMs_tmp = acid_read_vols(Vol{1,AxTM_index},Vol{1,1},-4); 
                 AxDKI_RBC_AxTMs_tmp = acid_read_vols(Vol{4,AxTM_index},Vol{1,1},-4); 
...
for inx_voxel = 1:size(voxel_folder,2)
    for AxTM_index = 1:numel(AxTM)
            ...
            results.bias_standardDKI_RBC(inx_voxel,AxTM_index,:) = 100*abs(Ground_Truth_Value - standardDKI_RBC_AxTMs(:,AxTM_index))/Ground_Truth_Value;
            ...
            results.std_standardDKI_RBC(inx_voxel,AxTM_index,:)  = 100*abs(AxDKI_RBC_AxTMs_STD(inx_voxel,AxTM_index,:))/Ground_Truth_Value;
    end    
end        
